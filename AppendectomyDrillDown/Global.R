...
# NOTE: Update this variable to...
#     = TRUE, load data from within the RProject (.RData Files)
#     = FALSE, load data from civis platform
useRData<-TRUE

loadRDataFromProject <- function() {
  # a. check existence --------------------------------------------------
  # b. load .rdata into global environment ------------------------------
  if(!(exists("encounter_df"))){
    load("~/benchwarmers/AppendectomyDrillDown/encounter_df.Rdata")
  }
  if(!(exists("hospital_df"))){
    load("~/benchwarmers/AppendectomyDrillDown/hospital_df.Rdata")
  }
  if(!(exists("full_df"))){
    load("~/benchwarmers/AppendectomyDrillDown/full_df.Rdata")
  }
}

loadDataFromCivis <- function() {
  # a. set up civis api key  ---------------------------------------------
  Sys.setenv(CIVIS_API_KEY='6156c21bdeeb714a16511f176b896d568787abf51deb1bd1165d1ee3685f1c2d')

  # b. load data from civis ----------------------------------------------
